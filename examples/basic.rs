use lifec::{prelude::{Host, Sequencer, Editor}, project::{Workspace, RunmdFile, Project}};
use lifec_shell::{Shell, Runmd};
...
fn main() {
    ...
    let world = Empty::compile_workspace(&workspace, files.iter(), None);

    let mut host = Host::from(world);
    host.link_sequences();
    host.open::<Empty, _>(Shell::<Runmd>::default());
}

#[derive(Default)]
struct Empty; 

impl Project for Empty {
    fn interpret(_: &lifec::prelude::World, _: &lifec::prelude::Block) {
        //
    }
}
